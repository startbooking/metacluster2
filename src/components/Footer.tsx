...
export const Footer = () => {
  ...
  // Don't render footer in dashboard routes
  if (isDashboardRoute) {
    return null;
  }
  ...
  return (
    <footer className="bg-gradient-to-br from-primary/5 to-accent/5 border-t border-border mt-16">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-gradient-llanos rounded-lg flex items-center justify-center">
                <span className="text-white font-bold">{appConfig?.app_name?.[0] || 'V'}</span>
              </div>
              <div>
                <h3 className="font-bold text-foreground">{appConfig?.app_name || 'Cluster de Turismo'}</h3>
                <p className="text-xs text-muted-foreground">de Villavicencio y el Meta Turismo</p>
              </div>
            </div>
            <p className="text-sm text-muted-foreground">
              {footerConfig?.company_description || 'Plataforma oficial del clúster de turismo de Villavicencio. Promoviendo el turismo sostenible y las empresas locales.'}
            </p>
              ...
              {(() => {
                const quickLinks = Array.isArray(footerConfig?.quick_links) 
                  ...
                  : typeof footerConfig?.quick_links === 'string' 
                    ...
                    : ['Directorio de empresas', 'Calendario de eventos', 'Mapa interactivo', 'Rutas turísticas', 'Guías de viaje'];
                
                return quickLinks.map((link: string, index: number) => (
                  <a key={index} href="#" className="block text-sm text-muted-foreground hover:text-primary transition-colors">
                    {link}
                  </a>
                ));
              })()}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {(() => {
                const businessLinks = Array.isArray(footerConfig?.business_links) 
                  ? footerConfig.business_links 
                  : typeof footerConfig?.business_links === 'string' 
                    ? JSON.parse(footerConfig.business_links || '[]') 
                    : ['Registrar empresa', 'Panel de control', 'Promocionar servicios', 'Estadísticas', 'Soporte técnico'];
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-muted-foreground">
                  {(footerConfig?.contact_address || appConfig?.address || 'Alcaldía de Villavicencio\nCarrera 35 #15-30\nVillavicencio, Meta')
                    .split('\n')
                    .map((line, index) => (
                      <p key={index}>{line}</p>
                    ))}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">{footerConfig?.contact_phone || appConfig?.phone || '+57 8 123 4567'}</span>
              </div>
              <div className="text-sm text-muted-foreground">
                {footerConfig?.contact_email || appConfig?.email || 'info@villavicencio-turismo.com'}
              </div>
            </div>
      ...
      <div className="bg-primary/20 backdrop-blur-md border-t border-border mt-1 p-4">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="text-sm text-muted-foreground">
            © 2025 <a className="text-brown font-bold" target="_blank" href="https://sactel.cloud">SACTel Cloud</a>. Todos los derechos reservados.
          </div>
